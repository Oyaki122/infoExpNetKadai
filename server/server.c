...
int main(int argc, char **argv)
{
  ...
  //   if (sendto(sock, ACK, n, 0, (sendtoAddr[dest - 1])->ai_addr,
  ...
  for (int i = 0; i < ACK_COUNT; i++)
  {
    ...
    {
      struct sockaddr_in *dest_sockaddr = (struct sockaddr_in *)(sendtoAddr[destNode[destIndex] - 1]->ai_addr);
      char *dest_ip = inet_ntoa(dest_sockaddr->sin_addr);
      printf("Send ACK to Node %d at IP: %s\n", destNode[destIndex], dest_ip);

      if (sendto(sock, ACK, strlen(ACK), 0, (struct sockaddr *)dest_sockaddr, sizeof(struct sockaddr)) != strlen(ACK))
      {
        perror("sendto");
      }
    }
